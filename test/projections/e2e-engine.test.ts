...
// Test aggregates
const invoiceAgg = { __aggregateType: 'invoice' as const };
...
function createMockSubscription(events: ProjectionEvent[]): IEventSubscription {
  ...
  return {
    poll: async (_cursor: Checkpoint, _batchSize: number) => {
      const batch = events.slice(index, index + 100);
      const lastSeq = batch.length > 0 ? batch[batch.length - 1].sequence : 0;
      index += batch.length;
      return {
        events: batch,
        nextCursor: { sequence: lastSeq + 1, timestamp: new Date().toISOString() },
...
describe('End-to-End Engine Test', () => {
  ...
  beforeEach(() => {
    ...
    // Create invoice summary projection using the actual API
    projection = createProjection<InvoiceSummaryState>('invoice-summary', {
      initialState: (id) => ({
        invoiceId: id,
        customerId: '',
        subtotal: 0,
  ...
  it('should process 5 events and produce correct final state', async () => {
    ...
    expect(cursor!.sequence).toBe(6); // Next cursor position after last event (sequence + 1)
  });
