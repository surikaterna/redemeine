...
describe('Lifecycle Routing', () => {
  ...
  let linkStore: InMemoryProjectionLinkStore;
  
  beforeEach(() => {
    store = new InMemoryProjectionStore<InvoiceState>();
    linkStore = new InMemoryProjectionLinkStore();
  });
  ...
  it('should expose usable unsubscribeFrom on handler context at runtime', async () => {
    ...
    const daemon = new ProjectionDaemon({ projection, subscription, store, linkStore });
