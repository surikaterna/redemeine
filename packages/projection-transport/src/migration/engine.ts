...
function receipt(
  ...
  command: ProjectionMigrationReceipt['command'],
  state: ProjectionMigrationState | null,
  mutated: boolean,
...
export class ProjectionMigrationEngine {
  ...
  constructor(private readonly ports: ProjectionMigrationEnginePorts) {
  ...
  async verifySources(manifest: ProjectionMigrationManifest): Promise<ProjectionMigrationReceipt> {
    ...
    try {
      ...
      return receipt(manifest, 'verify-sources', current.state, false, [message(error)]);
    }
  ...
  async replay(manifest: ProjectionMigrationManifest): Promise<ProjectionMigrationReceipt> {
    ...
    try {
      ...
      return receipt(manifest, 'replay', current.state, false, [message(error)]);
    }
  }
  ...
  async verify(manifest: ProjectionMigrationManifest): Promise<ProjectionMigrationReceipt> {
    ...
    try {
      ...
      return receipt(manifest, 'verify', current.state, false, [message(error)]);
    }
  ...
  private async require(
    manifest: ProjectionMigrationManifest,
    phases: readonly ProjectionMigrationState['phase'][],
    command: ProjectionMigrationReceipt['command']
  ): Promise<{ state: ProjectionMigrationState } | { receipt: ProjectionMigrationReceipt }> {
    const issues = [...validateProjectionMigrationManifest(manifest), ...(await this.ports.preflight.inspect(manifest, 'identity'))];
  ...
  private async transition(
    manifest: ProjectionMigrationManifest,
    command: ProjectionMigrationReceipt['command'],
    previous: ProjectionMigrationState | null,
    next: Omit<ProjectionMigrationState, 'revision'>
