...
import type { ProjectionMigrationManifest, ProjectionMigrationRangeJournal, ProjectionMigrationReplayPort, ProjectionMigrationSourceRange,
  ProjectionMigrationStatePort } from './types';
import { projectionMigrationRangeKey } from './validate';

const MAX_COMMITS = 100;
const MAX_BYTES = 8 * 1024 * 1024;

export async function scanProjectionMigrationRange(reader: ProjectionCompleteCommitRangeReader,
  range: ProjectionMigrationSourceRange, visit?: (commit: ProjectionSourceCommit) => Promise<void>): Promise<{ digest: `sha256:${string}`; count: number; bytes: number }> {
  ...
  while ((afterSequence ?? -1) < range.lastSequence) {
    const result = await reader.readCompleteRange({ sourceId: range.sourceId, afterSequence, throughSequence: range.lastSequence,
      maxCommits: MAX_COMMITS, maxBytes: MAX_BYTES });
    if (result.status !== 'complete' || result.commits.length === 0) throw new Error(`Complete bounded history unavailable for ${projectionMigrationRangeKey(range)}.`);
    for (const entry of result.commits) {
      ...
      digest.update(entry.commit); encodedBytes += entry.encodedByteLength; expected += 1;
...
export async function verifyProjectionMigrationSources(manifest: ProjectionMigrationManifest, reader: ProjectionCompleteCommitRangeReader,
  states: ProjectionMigrationStatePort, now: () => string): Promise<void> {
  ...
  for (const range of manifest.sourceRanges) {
    const key = projectionMigrationRangeKey(range); const row = existing.get(key);
    if (row && row.manifestDigest === manifest.manifestDigest && row.expectedDigest === range.expectedDigest
      && row.observedDigest === range.expectedDigest && row.commitCount === range.commitCount) continue;
    ...
    const journal: ProjectionMigrationRangeJournal = { migrationId: manifest.migrationId, manifestDigest: manifest.manifestDigest,
      rangeKey: key, expectedDigest: range.expectedDigest, observedDigest: observed.digest, commitCount: observed.count,
      encodedBytes: observed.bytes, verifiedAt: now() };
...
export function assertExactJournal(manifest: ProjectionMigrationManifest, rows: readonly ProjectionMigrationRangeJournal[]): void {
  ...
  if (actual.size !== manifest.sourceRanges.length || rows.length !== manifest.sourceRanges.length) throw new Error('Migration journal coverage is incomplete.');
  for (const range of manifest.sourceRanges) {
    const row = actual.get(projectionMigrationRangeKey(range));
    if (!row || row.manifestDigest !== manifest.manifestDigest
      || row.observedDigest !== range.expectedDigest || row.commitCount !== range.commitCount) throw new Error('Migration journal coverage mismatch.');
  }
}
...
export async function replayProjectionMigrationRanges(manifest: ProjectionMigrationManifest, reader: ProjectionCompleteCommitRangeReader,
  target: ProjectionMigrationReplayPort): Promise<void> {
  for (const range of manifest.sourceRanges) {
    const observed = await scanProjectionMigrationRange(reader, range);
    ...
    await scanProjectionMigrationRange(reader, range, async (commit) => {
      const result = await target.process(commit, { migrationId: manifest.migrationId, manifestDigest: manifest.manifestDigest,
        sourceId: commit.streamId, expectedSequence: commit.commitSequence === 0 ? null : commit.commitSequence - 1,
